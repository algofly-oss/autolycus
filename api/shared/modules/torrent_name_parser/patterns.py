...
patterns_allow_overlap = ["season", "episode", "language", "subtitles", "sbs"]
...
patterns["resolution"] = [
    (
        ...
        None,
        "lower",
    ),  # Generic pattern for resolutions like 480p, 720p, 1080p, etc.
    (r"(8K|7680{d}?x{d}?4320p?)".format(d=delimiters), "8K"),  # Pattern for 8K
    (r"(5K|5120{d}?x{d}?2880p?)".format(d=delimiters), "5K"),  # Pattern for 5K
    (
        r"(4K UHD|UHD|3840{d}?x{d}?2160p?)".format(d=delimiters),
        "2160p",
    ),  # Pattern for 4K UHD / 2160p
    (
        r"(4K|4096{d}?x{d}?2160p?)".format(d=delimiters),
        "4K",
    ),  # Pattern for 4K / Cinema 4K
    (
        r"(QHD|QuadHD|WQHD|2560{d}?x{d}?1440p?)".format(d=delimiters),
        "1440p",
    ),  # Pattern for QHD / 1440p
    (r"(2K|2048{d}?x{d}?1080p?)".format(d=delimiters), "2K"),  # Pattern for 2K
    (
        r"(Full HD|FHD|1920{d}?x{d}?1080p?)".format(d=delimiters),
        "1080p",
    ),  # Pattern for Full HD / 1080p
