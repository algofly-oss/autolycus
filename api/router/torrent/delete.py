...
async def delete_torrent(request: DeleteTorrentRequest, request_obj: Request):
    user_id = authenticate_user(request_obj.cookies.get("session_token")).decode("utf-8")
    torrent = await db.torrents.find_one(
        {"info_hash": request.info_hash, "user_id": ObjectId(user_id)},
        {"_id": True, "info_hash": True, "is_finished": True, "is_paused": True, "save_dir": True},
    )

    if torrent:
        if not torrent["is_finished"]:
            db.torrents.update_one({"_id": torrent["_id"]}, {"$set": {"is_paused": True}})
            redis.set(f"{user_id}/{torrent['info_hash']}/stop", 1)

            # Check the status in the database every 1 second
            while True:
                updated_torrent = await db.torrents.find_one({"_id": torrent["_id"]}, {"is_paused": True})
        ...
        emit(f"/stc/torrent-added-or-removed", {"action": "removed", "info_hash": request.info_hash}, user_id)
