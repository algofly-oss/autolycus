...
const FileItem = ({ item, handleFileAction, initialPath, handleItemClick }) => {
  ...
  const actions = [
    ...
    ...(getFileType(item.name) === "video" ? [{
      name: "Transcode", icon: RiMovie2Line, action: "transcode", subMenu: [
  ...
  //POST request to get transcoding progress
  useEffect(() => {
    if (item.is_transcoding) {

      axios.post(`${apiRoutes.transcodeProgress}?path=${initialPath}/${item.name}&stream=true`)
        .then((response) => {
          // setTranscodingProgress(response)
        }).catch((error) => {
          console.log(error)
        })
    }
  ...
  //Socket to get transcoding progress
  useEffect(() => {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="truncate flex w-full items-center justify-between">
          <div className="flex flex-col w-[95%]">
            ...
            {item.is_transcoding ? (
              <div className="mt-2">
                <ProgressBar progress={transcodingProgress.progress} showLabel={false} progress_height={'h-1.5'} />
                <div className="text-sm text-gray-500 mt-1">
                  {transcodingProgress.progress}% - Time Remaining {formatTimeRemaining(transcodingProgress.eta)}
                </div>
              </div>
            ) : (
